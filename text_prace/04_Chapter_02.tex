\chapter{Second chapter with mathematics \texorpdfstring{$\int\! f(x)\,\mathrm{d}x$}{int f(x) dx} in name}
...
\begin{equation*}\label{E}\tag{rovnice}
 \int\! f(x)\,\mathrm{d}x
\end{equation*}
odkaz na rovnici s ``tagem'' \eqref{E} -- pokud nechcete vzorci přidělit číslo, ale nějaký vlastní symbol, používejte
``hvězdičkovaná'' prostředí, tj. např. \verb+equation*+
