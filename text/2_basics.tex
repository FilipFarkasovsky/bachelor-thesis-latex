...
\subsection{ReliefF}
ReliefF is a filter-based feature selection algorithm designed to estimate the importance of features based on how well their values distinguish between instances of different classes. Unlike univariate methods, ReliefF is able to capture feature dependencies to some extent by considering the local neighborhood of each instance.

he algorithm works by repeatedly sampling an instance from the dataset and identifying its nearest neighbors: instances from the same class (nearest hits) and instances from different classes (nearest misses). For each attribute \(A\), ReliefF updates a weight according to how differently the attribute behaves between the sampled instance and its neighbors. Differences with nearest hits decrease the weight, while differences with nearest misses increase it. The contribution of misses is weighted by the class prior probabilities \(P(c)\), ensuring that their total influence is comparable to that of hits. A pseudocode description of the algorithm is provided by XXXXXMatheus Cezimbra Barbieri XXXXXXet al., and is presented in Algorithm~\ref{alg:relieff}.
